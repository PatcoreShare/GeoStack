#!/usr/bin/env python3
"""
GeoSatellite Scheduler - automatyczne pobieranie dla wszystkich województw.
Nazwa pliku: województwo-geosatellite_zMIN-MAX_timestamp.mbtiles
"""

import os
import time
import subprocess
import json
from datetime import datetime

# Konfiguracja z env vars (zachowana kompatybilność)
INTERVAL_SECONDS = int(os.getenv("INTERVAL_SECONDS", "86400"))
LAYER = os.getenv("LAYER", "GEOSATELLITE")
CONFIG_PATH = os.getenv("CONFIG_PATH", "/app/config.json")
MIN_ZOOM = os.getenv("MIN_ZOOM", "1")
MAX_ZOOM = os.getenv("MAX_ZOOM", "11")
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "/app/data")

def load_regions():
    """Wczytuje województwa z config.json."""
    try:
        with open(CONFIG_PATH, 'r') as f:
            config = json.load(f)
        return config.get('regions', {})
    except Exception as e:
        print(f"❌ Błąd wczytywania config.json: {e}")
        return {}

def run_job():
    """Pobiera mapy dla wszystkich województw."""
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    regions = load_regions()
    if not regions:
        print("❌ Brak województw w config.json - używam domyślnych bbox")
        # Fallback do env vars (jak było wcześniej)
        timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M")
        output_path = f"{OUTPUT_DIR}/{LAYER.lower()}_z{MIN_ZOOM}-{MAX_ZOOM}_{timestamp}.mbtiles"
        
        cmd = [
            "python", "main.py",
            "--layer", LAYER,
            "-b", os.getenv("BBOX_W", "14.0"), os.getenv("BBOX_S", "48.5"), 
                 os.getenv("BBOX_E", "24.5"), os.getenv("BBOX_N", "55.0"),
            "--min-zoom", MIN_ZOOM,
            "--max-zoom", MAX_ZOOM,
            "--output", output_path,
        ]
        
        print("=== GeoSatellite Scheduler (DOMYŚLNY) ===")
        print("Running:", " ".join(cmd))
        result = subprocess.run(cmd, capture_output=True, text=True)
        print(f"Job finished with return code: {result.returncode}")
        return
    
    # Pobieranie dla każdego województwa
    print("=== GeoSatellite Scheduler - WSZYSTKIE WOJEWÓDZTWA ===")
    success_count = 0
    
    for region_id, region_data in regions.items():
        timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M")
        # NOWA NAZWA: województwo-geosatellite_zMIN-MAX_timestamp.mbtiles
        region_name = region_data['name'].lower().replace(' ', '-').replace('ł', 'l').replace('ą', 'a')
        output_path = f"{OUTPUT_DIR}/{region_name}-{LAYER.lower()}_z{MIN_ZOOM}-{MAX_ZOOM}_{timestamp}.mbtiles"
        
        bbox = region_data['bbox']  # [W, S, E, N]
        cmd = [
            "python", "main.py",
            "--layer", LAYER,
            "-b", str(bbox[0]), str(bbox[1]), str(bbox[2]), str(bbox[3]),
            "--min-zoom", MIN_ZOOM,
            "--max-zoom", MAX_ZOOM,
            "--output", output_path,
        ]
        
        print(f"\n📍 {region_data['name']} (ID: {region_id})")
        print(f"📄 Plik: {os.path.basename(output_path)}")
        print("Running:", " ".join(cmd))
        
        result = subprocess.run(cmd, capture_output=True, text=True)
        print(f"Return code: {result.returncode}")
        
        if result.returncode == 0:
            print(f"✅ {region_data['name']} - SUKCES")
            success_count += 1
        else:
            print(f"❌ {region_data['name']} - BŁĄD")
            print(result.stderr[:300])
        
        time.sleep(3)  # Przerwa 3s między województwami
    
    print(f"\n📊 PODSUMOWANIE: {success_count}/{len(regions)} województw OK")

def main():
    while True:
        run_job()
        print(f"Sleeping for {INTERVAL_SECONDS} seconds ({INTERVAL_SECONDS/3600:.1f}h)...")
        time.sleep(INTERVAL_SECONDS)

if __name__ == "__main__":
    main()
